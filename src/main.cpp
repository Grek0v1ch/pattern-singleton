...
void init_trader() {
    Trader::instance().load_json("res/rates.json");
    std::cout << "Курсы валют на момент функции init_trader():\n";
    Trader::instance().print_rates();

    std::cout << "Переведем 10 рублей в доллары: "
              << Trader::instance().exchange_currency(10, "RUB", "USD") << std::endl;
    std::cout << "Переведем 10 долларов в рубли: "
              << Trader::instance().exchange_currency(10, "USD", "RUB") << std::endl;
    std::cout << "Переведем 5 рублей в тенге: "
              << Trader::instance().exchange_currency(10, "RUB", "KZT") << std::endl;
    std::cout << "Если мы попробуем перевести 10 евро в тенге, то у нас не выйдет: "
              << Trader::instance().exchange_currency(10, "EUR", "KZT");
    std::cout << std::endl;
}
...
void change_trader() {
    ...
    Trader::instance().add_exchange_rate("BYN", "RUB", 26.67);
    std::cout << "Изменим курс USD -> RUB : 75.25 на курс USD -> RUB : 1\n";
    Trader::instance().change_exchange_rate("USD", "RUB", 1);
